...
class DC_PreEmph(torch.nn.Module):
    ...
    def __init__(self, R=0.995):
        ...
        self.zPad = len(ir) - 1
        self.pars = torch.flipud(torch.tensor(
            ir, requires_grad=False, dtype=torch.FloatTensor.dtype)).unsqueeze(0).unsqueeze(0)
    ...
    def forward(
        ...
        output = torch.cat(
            (torch.zeros(output.shape[0], 1, self.zPad).type_as(output), output), dim=2)
        target = torch.cat(
            (torch.zeros(output.shape[0], 1, self.zPad).type_as(output), target), dim=2)

        output = torch.nn.functional.conv1d(
            output, self.pars.type_as(output), bias=None)  # [B, 1, T]
        target = torch.nn.functional.conv1d(
            target, self.pars.type_as(output), bias=None)  # [B, 1, T]
...
class STNSeparation(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x_t = torch.istft(X_t, n_fft=self.n_fft_2,
                          hop_length=self.hop_length_2)
        x_n = torch.istft(X_n, n_fft=self.n_fft_2,
                          hop_length=self.hop_length_2)
    ...
    def median_filter_freq(self, x, median_v):
        # x shape: (batch, freq, time)
        pad = median_v // 2
        x_padded = F.pad(x.transpose(-1, -2), (pad, pad),
                         mode='reflect').transpose(-1, -2)
    ...
    def compute_masks(self, x, beta_u, beta_l):
        ...
        mask[transition] = torch.sin(
            torch.pi/2 * (x[transition] - beta_l) / (beta_u - beta_l))**2
        return mask
