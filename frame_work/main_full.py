...
def inference(path_to_dataset, path_savedir, exp_dir_val):
    ...
    print(' > params amount: {:,d} | trainable: {:,d}'.format(
        amount, amount_train))
...
def train():
    ...
    copyfile(__file__, os.path.join(
        args['env']['expdir'], os.path.basename(__file__)))
    copyfile(cmd['config'], os.path.join(
        args['env']['expdir'], os.path.basename(cmd['config'])))
    # training
